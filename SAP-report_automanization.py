...
def formatting(sheet):
	...
	for j in range (1, (sheet.max_column+1)):		
		if j == (sheet.max_column):
			sheet.cell(row=2, column=j).value = 'Кол-во дней отсутствия решения (на {})'.format(date.today().strftime("%d.%m.%Y"))
...
def summary_filling (sheet, list):
	...
	for k in range(4):  #СР ЗНАЧ ПО ПОДИТОГАМ
		...
		NomStrPodItogov.append(list[k][4]+2)

	sheet.cell(row=3, column=13).value = 0
	for i in range (0, len(NomStrPodItogov)):
		sheet.cell(row=3, column=13).value += int(sheet.cell(row=NomStrPodItogov[i], column=13).value)
	sheet.cell(row=3, column=13).value = (int(sheet.cell(row=3, column=13).value)//len(NomStrPodItogov))


def show_all_rows(sheet):
	for row in sheet.iter_rows(min_row=1, max_row=sheet.max_row):
		if sheet.row_dimensions[row[0].row].hidden == True:
...
def result_by_criticality (sheet):
	statistic = []
	IPK = [0, 0, 0, 0, 0]
	# [0 - Общее кол-во задач по критичности, 1 - кол-во зарегистрированных задач, 2 - кол-во задач в работе, 3 - номер строки последней задачи, 4 - строка подытогов по критичности]
		...
		for i in range (1, sheet.max_row+1):
			if sheet.cell(row=i, column=9).value == krit:
				...
				if sheet.cell(row=(i+1), column=9).value != krit:
					IPK[3] = sheet.cell(row=(i+1), column=9).row
					IPK[4]=IPK[3]-IPK[0]+krit-1
		statistic.append(IPK);
		IPK = [0, 0, 0, 0, 0]
	return (statistic)
...
def create_result_rows(sheet, list, texts):
	...
	sheet.cell(row=1, column=1).value= 'Список проблем SAP на {} / SAP issues list on {}'.format(date.today().strftime("%d.%m.%Y"), date.today().strftime("%d.%m.%Y"))
...
def sign_rows(sheet):
	...
	for r in range(1,3):
		...
		for j in range(1, sheet.max_column+1):
				sheet.cell(row=n, column=j).font = Font(name='Times New Roman', size=12, bold=True, color = "000000")
				sheet.cell(row=n, column=j).alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
				sheet.cell(row=n, column=j).border = Border(left=Side(style='thin'), right=Side(style='thin'), top=Side(style='thin'), bottom=Side(style='thin'))
...
def standardization(sheet,list):
	PIT=[]
	viravnivanie = Alignment(horizontal='center', vertical='center', wrap_text=True)
	granici = Border(left=Side(style='thin'), right=Side(style='thin'), top=Side(style='thin'), bottom=Side(style='thin'))

	simpleFont = Font(name='Times New Roman', size=12)
	simpleFill = PatternFill(patternType='solid', start_color='ffffff', end_color='ffffff')
	
	TFont = Font(name='Times New Roman', size=24, bold=True, color = "000000")

	P_IFont = Font(name='Times New Roman', size=12, bold=True, color = "000000")
	IFill = PatternFill(patternType='solid', start_color='F5C243', end_color='F5C243')

	PIFill = PatternFill(patternType='solid', start_color='FEFF55', end_color='FEFF55')

	whiteFont=Font(name='Times New Roman', size=12, bold=True, color = "ffffff")
	greenFill=PatternFill(patternType='solid', start_color='4EAE5B', end_color='4EAE5B')
...
def main():
	...
	file='Список проблем SAP на {} - SAP issues list on {}.xlsx'.format(date.today().strftime("%d.%m.%Y"), date.today().strftime("%d.%m.%Y"))
	book.save(file)
